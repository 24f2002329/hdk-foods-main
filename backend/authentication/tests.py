...
class AuthenticationSecurityTests(TestCase):
    ...
    def test_jwt_token_lifetimes_and_rotation(self):
        ...
        token_expiration = datetime.datetime.fromtimestamp(access_token['exp'], tz=datetime.timezone.utc)
        self.assertAlmostEqual(
            (token_expiration - timezone.now()).total_seconds(),
            15 * 60,
            delta=30
        )
    ...
    def test_secure_cookie_refresh_token_storage(self):
        ...
        """
        # Login
        response = self.client.post("/api/auth/staff-login/", {
            "phone_number": "+919999999999",
            "password": "adminpassword123"
        })
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_logout_invalidates_token_and_clears_cookie(self):
        ...
        refresh_response = self.client.post("/api/auth/token/refresh/", {"refresh": refresh_token})
