...
def calculate_predicted_prep_time(product_ids):
    ...
    for product in products:
        base_prep = getattr(product, "base_prep_minutes", getattr(product, "preparation_time", 15))
    ...
    if config.peak_weekdays:
        try:
            peak_weekdays_list = [int(x.strip()) for x in config.peak_weekdays.split(",") if x.strip().isdigit()]
        except ValueError:
    ...
    if weekday in peak_weekdays_list:
        ...
        if config.peak_start_time <= config.peak_end_time:
            is_peak_hour = config.peak_start_time <= current_time <= config.peak_end_time
        else:
            is_peak_hour = current_time >= config.peak_start_time or current_time <= config.peak_end_time
