...
class _AdminCreateOrderScreenState extends State<AdminCreateOrderScreen> {
      ...
      if (res['found'] == true) {
        setState(() {
          ...
          final def = _savedAddresses.firstWhere(
            (e) => e['is_default'] == true,
            orElse: () =>
                _savedAddresses.isNotEmpty ? _savedAddresses.first : {},
          );
          if (def.isNotEmpty) {
  ...
  Future<void> _submit() async {
    ...
    final payload = {
      ...
      'pincode': _deliveryType == 'delivery'
                      ...
                      children: [
                        ...
                        DropdownButtonFormField<String>(
                          ...
                          items: const [
                            ...
                            DropdownMenuItem(
                              value: 'prepaid',
                              child: Text('Prepaid / Paid Online'),
                            ),
                          ],
                          onChanged: (v) => setState(() {
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: _saving
                          ? const Center(
                              child: HdkPreloader(width: 40, height: 40),
                            )
                          : ElevatedButton(
                              onPressed: _submit,
...
class _ProductPickerScreenState extends State<ProductPickerScreen> {
  ...
  Future<void> _customizeItem(Product product) async {
    ...
    if (_tempSelections.containsKey(product.id)) {
      final currentSels =
          _tempSelections[product.id]!['selections']
              as List<Map<String, dynamic>>;
      for (final group in product.modifierGroups) {
        final groupSels = currentSels
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () {
                    for (final group in product.modifierGroups) {
                      if (group.required &&
                          (tempSelections[group.id] ?? []).isEmpty) {
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final qty = selected != null
                          ? selected['quantity'] as int
                          : 0;
                      ...
                      children: [
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                '${_tempSelections.length} products selected',
                                style: const TextStyle(
                                  color: Colors.grey,
                                  fontSize: 12,
                                ),
