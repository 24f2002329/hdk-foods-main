...
class SentimentDashboardScreen extends StatefulWidget {
  ...
  @override
  State<SentimentDashboardScreen> createState() =>
...
class _SentimentDashboardScreenState extends State<SentimentDashboardScreen> {
  ...
  void _showIssueCouponDialog(String customerName, int customerId) {
    final codeController = TextEditingController(
      text: 'SORRY${math.Random().nextInt(900) + 100}',
    );
                  ...
                  onPressed: () async {
                    ...
                    try {
                      ...
                      messenger.showSnackBar(
                        SnackBar(
                          content: Text(
                            'Coupon ${codeController.text} successfully issued to $customerName!',
                          ),
    ...
    dishRatings.forEach((dish, ratings) {
      ...
      for (var rating in ratings) {
        if (rating <= 2) {
          ...
          if (consecutiveLow >= 3) {
            alerts.add(
              '🚨 Critical: "$dish" has received 3 consecutive low ratings ($rating★). Check recipe!',
            );
            break;
          }
        } else {
  ...
  Widget build(BuildContext context) {
    ...
    final filteredDishReviews = _productReviews.where((r) {
      ...
      final matchesRating =
          _selectedRatingFilter == 0 || r.rating == _selectedRatingFilter;
      return matchesSearch && matchesRating;
    }).toList();

    final filteredOrderReviews = _orderReviews.where((r) {
      final matchesSearch =
          r.orderNumber.toLowerCase().contains(_searchQuery.toLowerCase()) ||
          r.comment.toLowerCase().contains(_searchQuery.toLowerCase());
      final matchesRating =
          _selectedRatingFilter == 0 || r.rating == _selectedRatingFilter;
      return matchesSearch && matchesRating;
    ...
    final avgRating = _productReviews.isEmpty
        ? 0.0
        : _productReviews.map((r) => r.rating).reduce((a, b) => a + b) /
    ...
    final positivePct = _productReviews.isEmpty
        ? 0
        : (positiveCount / _productReviews.length * 100).round();
    final neutralPct = _productReviews.isEmpty
        ? 0
        : (neutralCount / _productReviews.length * 100).round();
    final negativePct = _productReviews.isEmpty
        ? 0
        : (negativeCount / _productReviews.length * 100).round();
      ...
      children: [
        ...
        LayoutBuilder(
          builder: (context, constraints) {
            ...
            if (isNarrow) {
              return Column(
                children: [lineChart, const SizedBox(height: 16), donutChart],
              );
            }
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      TextButton(
                        onPressed: () =>
                            setState(() => _showDishReviews = true),
                        child: Text(
                      ...
                      TextButton(
                        onPressed: () =>
                            setState(() => _showDishReviews = false),
                        child: Text(
              ...
              // Review list
              if (_showDishReviews
                  ? filteredDishReviews.isEmpty
                  : filteredOrderReviews.isEmpty)
                Padding(
                  padding: const EdgeInsets.symmetric(vertical: 40),
                  child: Center(
                    child: Text(
                ...
                ListView.separated(
                  ...
                  itemCount: _showDishReviews
                      ? filteredDishReviews.length
                      : filteredOrderReviews.length,
                  separatorBuilder: (ctx, idx) => const Divider(color: _stroke),
                  itemBuilder: (context, index) {
                    final rating = _showDishReviews
                        ? filteredDishReviews[index].rating
                        : filteredOrderReviews[index].rating;
                    final comment = _showDishReviews
                        ? filteredDishReviews[index].comment
                        : filteredOrderReviews[index].comment;
                    final customerName = _showDishReviews
                        ? filteredDishReviews[index].customerName
                        : filteredOrderReviews[index].customerName;
                    final customerPhone = _showDishReviews
                        ? filteredDishReviews[index].customerPhone
                        : filteredOrderReviews[index].customerPhone;
                    final customerId = _showDishReviews
                        ? filteredDishReviews[index].customerId
                        : filteredOrderReviews[index].customerId;
                    final orderNumber = _showDishReviews
                        ? filteredDishReviews[index].orderNumber
                        : filteredOrderReviews[index].orderNumber;
                    final createdAt = _showDishReviews
                        ? filteredDishReviews[index].createdAt
                        : filteredOrderReviews[index].createdAt;
                    final titleText = _showDishReviews
                        ? filteredDishReviews[index].productName
                        : 'Order Review';
                                  ...
                                  Wrap(
                                    ...
                                    children: [
                                      OutlinedButton.icon(
                                        ...
                                        onPressed: () => _showIssueCouponDialog(
                                          customerName,
                                          customerId,
                                        ),
                                      ),
                                      ElevatedButton.icon(
                                        ...
                                        onPressed: () {
                                          ScaffoldMessenger.of(
                                            ...
                                            SnackBar(
                                              content: Text(
                                                'Opening chat with phone: $customerPhone',
                                              ),
                                            ),
                                          );
                                        },
      ...
      LineChartData(
        gridData: FlGridData(
          ...
          getDrawingHorizontalLine: (value) =>
              const FlLine(color: _stroke, strokeWidth: 1),
        ),
